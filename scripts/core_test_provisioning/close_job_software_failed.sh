...
mosquitto_pub --cafile ${CA} --cert ${CERT} --key ${KEY} -h ${iotHost} -q 1 -p 8883 -t "\$aws/things/${environmentPrefix}_${thingId}/jobs/${jobId}/update" -i "${environmentPrefix}_${thingId}" -m "{ \"status\":\"FAILED\",\"statusDetails\": {\"environment_prefix\": \"$environmentPrefix\" , \"step\": \"command failed\", \"install_status\": \"failed_install\" } }" -d