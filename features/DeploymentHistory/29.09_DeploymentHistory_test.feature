@29.09_DeploymentHistory_test
Feature: Deployment History

  #script

  #https://jira.springlab.enel.com/browse/EXEGIP2-1260

  #GOAL: Install a new package and verify if the icon of the Package Installed is green
  ...
  Scenario: Deployment History 29.9
   ...
   # And I check the presence of DevicesPage.confirmFilters Element and then i click it
    ...
    And I check the presence of DevicesDetailsPage.version32OptionToSelect Element and then i click it
    ...
    #4b. The compleation job message on the script must be this: { \"status\":\"SUCCEEDED\",\"statusDetails\": \{\"environment_prefix\": \"$environmentPrefix\" , \"step\": \"command succeeded\", \"install_status\": \"complete_install\",\"gateway_sw\": \"<app correctly installed>\" }}"
    Then I save the job ID from the job document received and I verify that the inbound message have the specified format
    And I run the script "close_job_software_complete_29_09", with the job Id retrieved, on "Test_Concept-17" in the project "esol_ap29551_qa"
    Then I kill the notify_job command still opened
    ...
    #4c. Verify on the Deployments History details that only the apps indicated on the message are showed as installed
    Then I click HomePage.hamburgerMenuIcon
    Then I click SidebarPage.devicesOption
    And I wait the progress bar
    ...
    And I check the presence of DeploymentHistoryDeviceDetailsPage.packageInstallationOutcome2909
  ...
  # Open Gateway's detailed page on Devices section and check the deployment is present on Deployment History section - ALREADY DONE IN STEP 4c
