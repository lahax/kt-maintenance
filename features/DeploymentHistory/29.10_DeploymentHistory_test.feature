@29.10_DeploymentHistory_test
Feature: Deployment History

  #script

  #https://jira.springlab.enel.com/browse/EXEGIP2-1262

  #GOAL: Install a new package and verify if the icon of the package installed is red
  ...
  Scenario: Deployment History 29.10
    ...
    # Select the package to install in "App name" section, then select the desired version.
    ...
    And I check the presence of DevicesDetailsPage.version4OptionToSelect Element and then i click it
    #Click on "Update" to start the updating process:
    #try package: APP1 V4.0 GW have to be: GATEWAY TYPE regression-eig
    ...
    #4b. The compleation job message on the script must be this:
    #{ \"status\":\"SUCCEEDED\",\"statusDetails\": {\"environment_prefix\": \"$environmentPrefix\" , \"step\": \"command succeeded\", \"install_status\": \"failed_install\",\"gateway_sw\": \"<app correctly installed>\" }}"
    Then I save the job ID from the job document received and I verify that the inbound message have the specified format
    And I run the script "close_job_software_failed_29_10", with the job Id retrieved, on "Test_Concept-17" in the project "esol_ap29551_qa"
    Then I kill the notify_job command still opened
    And I wait 120 seconds after script

    #4c. Verify on the Deployments History details that only the apps indicated on the message are showed as failed
    Then I click HomePage.hamburgerMenuIcon
    Then I click SidebarPage.devicesOption
    And I wait the progress bar
    ...
    And I check the presence of DevicesDetailsPage.iconFailed
    And I check the presence of DeploymentHistoryDeviceDetailsPage.firstDetailsiconInTheTable Element and then i click it
    And I check the presence of DeploymentHistoryDeviceDetailsPage.deploymentStatusFailed
    And I check the presence of DeploymentHistoryDeviceDetailsPage.packageInstallationOutcome2910
    ...
    And I check the presence of DeploymentsHistoryPage.iconFailed
