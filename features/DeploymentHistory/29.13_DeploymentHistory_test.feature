@29.13_DeploymentHistory_test
Feature: Deployment History

  #script

  #https://jira.springlab.enel.com/browse/EXEGIP2-1265

  #GOAL: Install a new package and verify if the icon of the package installed is yellow
  ...
  Scenario: Deployment History 29.13
    ...
    And I check the presence of DevicesDetailsPage.version6OptionToSelect Element and then i click it
    ...
    #4b. The compleation job message on the script must be this:{ \"status\":\"SUCCEEDED\",\"statusDetails\":{\"environment_prefix\": \"$environmentPrefix\" , \"step\": \"command succeeded\", \"install_status\": \"inprogress_install\",\"gateway_sw\": \"<app correctly installed>\" }}"
    Then I save the job ID from the job document received and I verify that the inbound message have the specified format
    And I run the script "close_job_software_inprogress_29_13", with the job Id retrieved, on "Test_Concept-17" in the project "esol_ap29551_qa"
    Then I kill the notify_job command still opened
    And I wait 120 seconds after script
    #TODO TA improvement: get one elegant method to control stepfunction progress, no toast msg
    ...
    And I check the presence of DevicesDetailsPage.iconPending
    And I check the presence of DeploymentHistoryDeviceDetailsPage.firstDetailsiconInTheTable Element and then i click it
    And I check the presence of DeploymentHistoryDeviceDetailsPage.deploymentStatusInProgress
    And I check the presence of DeploymentHistoryDeviceDetailsPage.packageInstallationOutcome2913
    ...
    And I check the presence of DeploymentsHistoryPage.iconPending
