@ThingFirmwareUpdate_Test_25.2
  Feature: Thing Firmware Update

   # https://confluence.springlab.enel.com/display/EXEGIPL/%5BEGIP+2021%5D+%5BEXEGIP2-35%5D+OCPP+Software+Update+-+Test+Guide+-+25.2
   # GOAL: ACTIVATE OCPP THING (with software registered in SW Catalog)
   # Step 1
    #Login on DMC
    Background:
    ...
    Scenario: ThingFirmwareUpdate_Test_25.2
    ...
    """
{
    ...
    #13 -  Run registration script
      Then I launch command "bash ./register_thing.sh ax3v62h9b8tp2.iot.eu-central-1.amazonaws.com esol_ap29551_qa Test_Concept-51 thing1 ./payload_25_02.txt" on shell
    #14a – Wait for the registration message: the inbound message must match payload file.
      And I print "payload_25_02" used in script
...
{"status": "SUCCEEDED", "message": "end of registration command", "device_id": "%s"}
      ...
      And I check the presence of ThingsPage.labelSoftwareAppType

      Then I click HomePage.hamburgerMenuIcon
      Then I click SidebarPage.softwareManagerDropDownOption
      And I click SidebarPage.deploymentsHistoryOption
      Then I check the presence of DeploymentsHistoryPage.lastDeployThingConcept51App1
