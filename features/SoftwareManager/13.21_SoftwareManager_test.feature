...
#GOAL: Verify the Campaign is correctly executed on the devices listed
  ...
  Scenario: Software_manager_Test_13.21
  ...
  # STEP 3: Select more than one gateway with the same device type (i.e. regression-eig), then click on the button on the right down of the page
    ...
    Then I check the presence of CampaignManagementView.regressionEigCuOption Element and then i click it
    ...
    And I wait the progress bar
    And I choice two device ID: "Gateway-Concept-3" and "Gateway-Concept-4"
    And I wait 10 seconds

    #STEP 4: If the devices are simulated through script - (For each Gateway run script notify_job)
    ...
    Then I run the notify_job_script on "Gateway-Concept-3", in the project "esol_ap29551_qa"
    Then I run the notify_job_script on "Gateway-Concept-4", in the project "esol_ap29551_qa"

    #STEP 5: Select firmware update and fill data required
    ...
    #Step 6: If device is simulated through script:
    #Execute the close_job.sh script to close the step function with success
    #close_job.sh ax3v62h9b8tp2.iot.eu-central-1.amazonaws.com <env> <gw_id> <job_id>
    #Es: ./close_job.sh   ax3v62h9b8tp2.iot.eu-central-1.amazonaws.com  esol_ap29551_qa  GwTest  esol_ap29551_qa-2201260931-167b41d0-a7cb-40e4-a6eb-addd67bc7125
    #(the job_id can be retrieved by the MQTT message received by notify_job script)
    Then I check the presence of CampaignManagementView.updateButton Element and then i click it
    Then I save the job ID from the job document received from the gateway "Gateway-Concept-3"
    Then I save the job ID from the job document received from the gateway "Gateway-Concept-4"

    And I run the script: "close_job_software", with the job Id retrieved, on "Gateway-Concept-3" in the project "esol_ap29551_qa"
    And I run the script: "close_job_software", with the job Id retrieved, on "Gateway-Concept-4" in the project "esol_ap29551_qa"
    ...
    And I check and click the last icon document of the entry named "Gateway-Concept-3"
    ...
    Then I select the details icon of the entry "Gateway-Concept-3"
    ...
    Then I check the presence of DeploymentsHistoryPage.gatewayOptionFilter Element and then i click it
    Then I check the presence of DeploymentsHistoryPage.confirmFilterOptionBtn Element and then i click it
    Then I select the details icon of the entry "Gateway-Concept-4"
    And I check the presence of DevicesDetailsPage.deploymentDetailsTitle
    Then I check the presence of DevicesDetailsPage.bannerCompleteInstall

    #STEP 9: Open Gateway's detailed page on Devices section and check the deployment is present on Deployment History section
    Then I click HomePage.hamburgerMenuIcon
    Then I click SidebarPage.devicesOption
    #Then I check the presence of DevicesPage.filterBox Element and then i click it
    #And I check the presence of DevicesPage.checkboxNeverConnected Element and then i click it
    #And I check the presence of DevicesPage.confirmFilters Element and then i click it
    Then I input "Gateway-Concept-3" in DevicesPage.searchBox
    Then I click on details icon of the device "Gateway-Concept-3"
    ...
    Then I input "Gateway-Concept-4" in DevicesPage.searchBox
    Then I click on details icon of the device "Gateway-Concept-4"
    Then I check the presence of DevicesDetailsPage.tabDeploymentsHistory Element and then i click it
    Then I check the presence of DeploymentHistoryDeviceDetailsPage.firstDetailsiconInTheTable Element and then i click it
    Then I check the presence of package to install with the name "APP1" and version "1.0"
    ...
    #{ \"status\":\"SUCCEEDED\",\"statusDetails\": {\"environment_prefix\": \"$environmentPrefix\" , \"step\": \"command succeeded\", \"install_status\": \"partial_install\",\"gateway_sw\": \"<app correctly installed>\" } }"
    ...
    #Step3
    #And I choice two device ID: "GatewayConceptGG" and "GatewayConceptGG2"
    Then I check the presence of CampaignManagementView.gatewayConcept3 Element and then i click it
    And I wait 3 seconds
    Then I check the presence of CampaignManagementView.gatewayConcept4 Element and then i click it
    And I wait 10 seconds
    ...
    And I run the script: "close_job_software_partial", with the job Id retrieved, on "Gateway-Concept-3" in the project "esol_ap29551_qa"
    And I run the script: "close_job_software_partial", with the job Id retrieved, on "Gateway-Concept-4" in the project "esol_ap29551_qa"
    Then I kill the notify_job command still opened
    Then I wait 3 minutes after script
    Then If present i click on Enter Button
    Then I click HomePage.hamburgerMenuIcon
    ...
    #And I click SidebarPage.stepFunctionSubOption
    #And I check the presence of StepFunctionPage.stepFunctionLandingPageTitle
    #And I check and click the last icon document of the entry named "Gateway29-Concept3"
    #And I check the presence of StepFunctionPage.succeededInDetailsStepFunction
    #Then I check the presence of HomePage.hamburgerMenuIcon Element and then i click it
    Then I check the presence of SidebarPage.softwareManagerDropDownOption Element and then i click it
    Then I check the presence of SidebarPage.deploymentsHistoryOption Element and then i click it
    ...
    #STEP 11: Repeat step 3 - 8, where:
        ...
        #the job closure message on the script is:
        #{ \"status\":\"SUCCEEDED\",\"statusDetails\": {\"environment_prefix\": \"$environmentPrefix\" , \"step\": \"command succeeded\", \"install_status\": \"failed_install\",\"gateway_sw\": \"\" } }"
    ...
    Then I save the job ID from the job document received from the gateway "Gateway-Concept-3"
    Then I save the job ID from the job document received from the gateway "Gateway-Concept-4"
    And I run the script: "close_job_software_failed", with the job Id retrieved, on "Gateway-Concept-3" in the project "esol_ap29551_qa"
    And I run the script: "close_job_software_failed", with the job Id retrieved, on "Gateway-Concept-4" in the project "esol_ap29551_qa"
    Then I kill the notify_job command still opened
    Then I wait 3 minutes after script
    Then If present i click on Enter Button
    Then I click HomePage.hamburgerMenuIcon
